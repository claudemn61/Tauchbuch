...
const TAUCH_TILE_OPTIONS = [
  ...
  { key: "blei", label: "Blei", icon: "⚖️", get: d => d.blei ? d.blei + " kg" : "—", rawGet: d => d.blei || "", save: v => ({ blei: v }) },
  { key: "tgNr", label: "TG-Nr.", icon: "#", get: d => d.tgNr || "—", rawGet: d => d.tgNr || "", save: v => ({ tgNr: v }) },
  { key: "volumen", label: "Volumen", icon: "🛢", get: d => d.volumen || "—", rawGet: d => d.volumen || "", save: v => ({ volumen: v }) },
  { key: "flasche", label: "Flasche", icon: "🔩", get: d => d.flasche || "—", rawGet: d => d.flasche || "", save: v => ({ flasche: v }) },
  { key: "nitrox", label: "Nitrox", icon: "💨", get: d => d.nitrox || "—", rawGet: d => d.nitrox || "", save: v => ({ nitrox: v }) },
...
// ── Detail view ──────────────────────────────────────────────────────────
function DetailContent({ d, dives, setDives, setSelected, setView, saveDive, confirmDelete, setConfirmDelete, returnTo }) {
  ...
  return (
    <div style={{maxWidth:480,margin:"0 auto",padding:"0 0 32px",background:"#040e20",minHeight:"100vh",color:"#e8f4fd",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif"}}>
      ...
      <div style={{padding:"0 16px"}}>
        ...
        {/* Bewertung — direkt unter dem Titel */}
        ...
        {/* Bemerkungen — direkt unter der Bewertung */}
          ...
          return (
            <div onClick={()=>setBulkEditOpen(false)}
              ...
              <div onClick={e=>e.stopPropagation()}
                ...
                {field("Anzug", "anzug")}
                {field("Blei (kg)", "blei")}
                {field("Flasche", "flasche")}
                {field("Volumen", "volumen")}
                {field("Nitrox (Ja/Nein)", "nitrox")}
                {field("Buddy", "buddy")}
                <div style={{marginBottom:12}}>
